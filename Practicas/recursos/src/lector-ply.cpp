...
class LectorPLY
{
   ...
   void leerVertices  ( std::vector<Tupla3f> & vertices  ) ;
   void leerCaras     ( std::vector<Tupla3i> & caras   ) ;
   void leerRestoLinea() ;
...
void LectorPLY::abrirArchivo( const std::string & p_nombre_archivo )
{
   ...
   src.open( nom_archivo.c_str() ) ; // abrir (¿en modo lectura?)

   if ( ! src.is_open() )
      error( "no puedo abrir el archivo para lectura (no existe o no es legible)." ) ;
...
void LectorPLY::leerVertices( std::vector<Tupla3f> & vertices  )
{
   string token ;
   ...
   for( unsigned long long iv = 0 ; iv < num_vertices ; iv++ )
   {
      ...
      src >> x >> y >> z ;
      leerRestoLinea();
      vertices[iv] = Tupla3f( float(x), float(y), float(z) );
   }
...
void LectorPLY::leerCaras( std::vector<Tupla3i> & caras   )
{
   string        token ;
   constexpr int nvc = 3 ;
   ...
   for( unsigned long long ifa = 0 ; ifa < num_caras ; ifa++ )
   {
      ...
      Tupla3i cara_leida ; assert( nvc == 3 ); //;-)

      for ( unsigned ivc = 0 ; ivc < nvc ; ivc++ )
      {
         src >> cara_leida[ivc] ;
         if ( (int)num_vertices <= cara_leida[ivc] )
